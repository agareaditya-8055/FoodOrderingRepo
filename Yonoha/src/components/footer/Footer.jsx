const Footer = () => {
  return (
    <>
      <footer className="w-full bg-gray-900 flex justify-center text-white">
        <div className="max-w-[1100px] w-[1100px] py-12 px-0 relative">
          <section className="flex items-center justify-between w-full mb-8">
            <span className="text-logoColor text-4xl font-bold mb-2 sm:mb-0">
              YONOHA
            </span>
            <div className="mt-3"></div>
            <section className="selector flex w-[252px] h-[38px] justify-between items-center">
              <div className="India w-100 h-full flex items-center justify-evenly p-3 border border-gray-300 rounded-md">
                <img
                  className="w-[22px] h-[14px]"
                  src="data:image/png;base64,"
                  alt=""
                />
                <p className="text-base">India</p>
                <i
                  className="fa-solid fa-chevron-down w-4 h-4"
                  style={{ color: "#ffffff" }}
                ></i>
              </div>
              <div className="English w-[110] h-full flex items-center justify-evenly p-1 border border-gray-300 rounded-md">
                <i
                  className="fa-solid fa-globe w-[18] h-[18] text-base"
                  style={{ color: "#ffffff" }}
                ></i>
                <p className="text-base leading-[2px] ml-2 mr-1 font-normal">
                  English
                </p>
                <i
                  className="fa-solid fa-chevron-down w-4 h-4 "
                  style={{ color: "#ffffff" }}
                ></i>
              </div>
            </section>
          </section>
          <div className="main-footer w-full relative flex flex-wrap justify-between">
            <div className="foot1 relative w-full max-w-[18.3333%]">
              <h6 className="text-xs font-extrabold tracking-wider uppercase mb-3">
                ABOUT Yonoha
              </h6>
              <nav>
                <a className="no-underline" href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">
                    Who We Are
                  </p>
                </a>
                <a className="no-underline" href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">Blog</p>
                </a>
                <a className="no-underline" href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">
                    Work With Us
                  </p>
                </a>
                <a className="no-underline" href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">
                    Investor Relations
                  </p>
                </a>
                <a className="no-underline" href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">
                    Report Fraud
                  </p>
                </a>
                <a className="no-underline" href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">
                    Contact Us
                  </p>
                </a>
              </nav>
            </div>
            <div className="foot2">
              <h6 className="text-xs font-extrabold tracking-wider uppercase mb-3">
                YONOHAVERSE
              </h6>
              <nav>
                <a href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">Yonoha</p>
                </a>
                <a href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">Yonoit</p>
                </a>
                <a href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">
                    Feeding India
                  </p>
                </a>
                <a href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">
                    Hyperpure
                  </p>
                </a>
                <a href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">
                    Yonohaland
                  </p>
                </a>
              </nav>
            </div>
            <div className="foot3">
              <div className="foot3-1 mb-7">
                <h6 className="text-xs font-extrabold tracking-wider uppercase mb-3">
                  FOR RESTAURANTS
                </h6>
                <nav>
                  <a href="#">
                    <p className="text-sm leading-5 text-gray-400 my-2">
                      Partner With Us
                    </p>
                  </a>
                  <a href="#">
                    <p className="text-sm leading-5 text-gray-400 my-2">
                      Apps For You
                    </p>
                  </a>
                </nav>
              </div>
              <div className="foot3-2">
                <h6 className="text-xs font-extrabold tracking-wider uppercase mb-3">
                  FOR ENTERPRISES
                </h6>
                <nav>
                  <a href="#">
                    <p className="text-sm leading-5 text-gray-400 my-2">
                      Yonoha For Enterprise
                    </p>
                  </a>
                </nav>
              </div>
            </div>
            <div className="foot4">
              <h6 className="text-xs font-extrabold tracking-wider uppercase mb-3">
                LEARN MORE
              </h6>
              <nav>
                <a href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">
                    Privacy
                  </p>
                </a>
                <a href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">
                    Security
                  </p>
                </a>
                <a href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">Terms</p>
                </a>
                <a href="#">
                  <p className="text-sm leading-5 text-gray-400 my-2">
                    Sitemap
                  </p>
                </a>
              </nav>
            </div>
            <div className="foot5 ">
              <h6 className="text-xs font-extrabold tracking-wider uppercase mb-3">
                SOCIAL LINKS
              </h6>
              <div className="icons flex justify-start h-[22px] w-[201px]">
                <a
                  className="no-underline mr-2 text-lg"
                  target="_blank"
                  href="https://www.linkedin.com/in/jainrithik/"
                >
                  <i
                    className="fa-brands fa-linkedin"
                    style={{ color: "#ffffff" }}
                  ></i>
                </a>
                <a
                  className="no-underline mr-2 text-lg"
                  target="_blank"
                  href="https://www.linkedin.com/in/jainrithik/"
                >
                  <i
                    className="fa-brands fa-instagram"
                    style={{ color: "#ffffff" }}
                  ></i>
                </a>
                <a
                  className="no-underline mr-2 text-lg"
                  target="_blank"
                  href="https://www.linkedin.com/in/jainrithik/"
                >
                  <i
                    className="fa-brands fa-twitter"
                    style={{ color: "#ffffff" }}
                  ></i>
                </a>
                <a
                  className="no-underline mr-2 text-lg"
                  target="_blank"
                  href="https://www.linkedin.com/in/jainrithik/"
                >
                  <i
                    className="fa-brands fa-youtube"
                    style={{ color: "#ffffff" }}
                  ></i>
                </a>
                <a
                  className="no-underline mr-2 text-lg"
                  target="_blank"
                  href="https://www.linkedin.com/in/jainrithik/"
                >
                  <i
                    className="fa-brands fa-facebook"
                    style={{ color: "#ffffff" }}
                  ></i>
                </a>
              </div>
              <div className="blank1 mt-6"></div>
              <section className="store ">
                <a href="#">
                  <div className="storeimg relative max-w-full w-[137px] h-[40px] overflow-hidden">
                    <img
                      className="w-full h-full object-cover"
                      src="https://b.zmtcdn.com/data/webuikit/9f0c85a5e33adb783fa0aef667075f9e1556003622.png"
                      alt="App store"
                    />
                  </div>
                </a>
                <div className="blank2 mt-3"></div>
                <a href="#">
                  <div className="storeimg relative max-w-full w-[137px] h-[40px] overflow-hidden">
                    <img
                      className="w-full h-full object-cover"
                      src="https://b.zmtcdn.com/data/webuikit/23e930757c3df49840c482a8638bf5c31556001144.png"
                      alt="Playstore"
                    />
                  </div>
                </a>
              </section>
            </div>
          </div>
          <hr className="h-line border-t border-gray-300 mt-12" />
          <p className="copyright text-xs text-gray-500 leading-4 mt-6">
            By continuing past this page, you agree to our Terms of Service,
            Cookie Policy, Privacy Policy, and Content Policies. All trademarks
            are properties of their respective owners. 2008-2023 © Yonoha Ltd.
            All rights reserved.
          </p>
        </div>
      </footer>
    </>
  );
};

export default Footer;
